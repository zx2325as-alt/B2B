...
def main():
    ...
    # 1. Backend
    backend_cmd = f"python -m uvicorn app.main:app --reload --host {settings.HOST} --port {settings.BACKEND_PORT}"
    run_service(backend_cmd, f"BtB Backend API (Port {settings.BACKEND_PORT})")
    
    time.sleep(2) # Wait a bit for backend

    # 2. Admin
    admin_cmd = f"python -m streamlit run app/web/admin.py --server.port {settings.ADMIN_PORT} --server.address {settings.HOST}"
    run_service(admin_cmd, f"BtB Admin Dashboard (Port {settings.ADMIN_PORT})")

    # 3. Chat UI
    chat_cmd = f"python -m streamlit run app/web/chat_ui.py --server.port {settings.CHAT_PORT} --server.address {settings.HOST}"
