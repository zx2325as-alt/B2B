...
{{
  "perspectives": [
    {{
      ...
      "confidence": 0.0,
      "grounded": true,
      "final_thought": "一段连贯的最终判断：这个视角此刻最可信的「想法/读到了什么」。这一段必须是已经自检过、去掉脑补、并权衡过另一种可能后的结论——它就是给用户看的那段话，连贯成话、不分点、第一人称",
      "alternative": "这句话最强的一个'另一种解读'（一句）；若你的主判断明显更站得住则留空，若两种势均力敌则必须写出并相应调低 confidence",
...
一次想透·直接给终版（关键：你要在这一次里完成"分析+自我复核+权衡反面+整合"，不要留草稿）：
- **先自检再落笔**：你的 inner_monologue / emotions / tags 有没有超出"这句话本身能支撑"的范围？把脑补降级——单凭一句"你好呀"推不出"约饭/底细/试探"这类剧情。宁可写"信息有限、暂判不准"，也不要硬编笃定动机。
- **想一个最强的"另一种可能"**(alternative)：如果它和你的主判断势均力敌，confidence 要相应调低，并在 alternative 里点明"取决于什么"；若主判断明显更站得住，alternative 留空。
- **内部必须一致**：final_thought 谦虚，则 emotions 的强度(score)要相应回落、tags 不能再断言被你自己否掉的动机。绝不允许"final_thought 说说不准、情绪标签却笃定地说他在试探"。
- final_thought 就是最终结论本身，不是过程——用户只看这一段，它必须已经是收敛后的判断。
