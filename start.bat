...
echo [0/3] Cleaning old dev processes on ports 5173 / 8001 / 7474 / 7687...
powershell -NoProfile -Command "$ports = 5173,8001,7474,7687; foreach ($port in $ports) { Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique | ForEach-Object { Stop-Process -Id $_ -Force -ErrorAction SilentlyContinue } }"
